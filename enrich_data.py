...
# -*- coding: utf-8 -*-
"""
Enriches data.js with:
1. Complete dual-generation (40-60 & 25-35) + harmony metadata for each spot
2. SPOTS_CATALOG: Comprehensive catalog of unique spots
3. PRESET_ITINERARIES: Curated templates for Official, Senior (40-60), Young (25-35), and Cross-Gen
4. CROSS_GEN_GUIDELINES in TRAVEL_TIPS
"""
...
def get_gen_key(item_id, name_zh):
    name = name_zh.lower()
    if "toyota" in name or "豐田" in name:
        if "還車" in name or "返却" in name:
            return "toyota_return"
        return "toyota"
    if "潮崎" in name or "shiozaki" in name:
        return "san_a_shiozaki"
    if "ocean tree" in name or "海之樹" in name:
        return "ocean_tree"
    if "costco" in name or "好市多" in name:
        return "costco"
    if "煙火" in name or "美國村" in name:
        return "american_village"
    if "魚市場" in name:
        return "itoman_fish"
    if "iias" in name:
        return "iias"
    if "dmm" in name or "水族館" in name:
        return "dmm_aquarium"
    if "美麗sun" in name or "海灘" in name:
        return "chura_sun_beach"
    if "漁民食堂" in name:
        return "itoman_gyomin"
    if "護國神社" in name:
        return "gokoku_shrine"
    if "波上宮" in name:
        return "naminoue_shrine"
    if "dfs" in name or "迪斐世" in name:
        return "dfs"
    if "國際通" in name:
        return "kokusai_dori"
    if "田舍" in name:
    ...
    if "eibun" in name:
        return "eibun_soba"
    if "sports depo" in name or "天久" in name:
        return "sports_depo"
    if "瀨長島" in name or "umikaji" in name:
        return "senagajima"
    if "maxvalu" in name or "美思佰樂" in name:
        return "maxvalu"
    if "達磨寺" in name or "西來院" in name:
        return "daruma_temple"
    if "7-eleven" in name or "首里" in name:
        return "seven_eleven_shuri"
    if "敘敘苑" in name:
        return "jojoen"
    if "parco city" in name or "浦添西海岸" in name:
        return "parco_city"
    if "workman" in name:
        return "workman_plus"
    if "かつ乃屋" in name or "main place" in name:
        return "katsunoya"
    if "小やじ" in name or "koyaji" in name:
        return "tonkatsu_koyaji"
    if "道之驛" in name or "豐崎" in name:
        return "toyosaki_roadside"
    if "永旺" in name or "aeon" in name:
        return "aeon_naha"
    if "通堂" in name:
        return "tondo_ramen"
    if "機場" in name:
...
# Now read existing data.js
with open("data.js", "r", encoding="utf-8") as f:
    orig_code = f.read()

# Parse items from data.js
# We will inject `generation: { ... }` into each item in SCHEDULE_ITEMS
def inject_generation(match):
    full_item = match.group(0)
    name_zh_m = re.search(r"nameZh:\s*\"([^\"]+)\"", full_item)
    id_m = re.search(r"id:\s*\"([^\"]+)\"", full_item)
    if not name_zh_m or not id_m:
        return full_item
    name_zh = name_zh_m.group(1)
    item_id = id_m.group(1)
    gen_key = get_gen_key(item_id, name_zh)
    gen_obj = GENERATION_DATA.get(gen_key, GENERATION_DATA["ocean_tree"])
    
    # Format generation javascript snippet
    gen_js = f"""    generation: {json.dumps(gen_obj, ensure_ascii=False, indent=6).strip()},"""
    
    # Insert before tips: or tags: or desc:
    if "desc:" in full_item:
        full_item = full_item.replace("desc:", f"{gen_js}\n    desc:", 1)
    return full_item

# Regex to match each object in SCHEDULE_ITEMS
item_pattern = re.compile(r"\{\s*\n\s*id:\s*\"d\d+-\d+\",.*?\n\s*\}", re.DOTALL)
new_code = item_pattern.sub(inject_generation, orig_code)

# Prepare SPOTS_CATALOG:
# Unique distinct spots across Okinawa
SPOTS_CATALOG = [
    {
        "id": "spot-toyota",
        ...
        "generation": GENERATION_DATA["toyota"]
    },
    {
        ...
        "defaultDurationMinutes": 60,
        "tags": ["南部超大型超市", "隨車補給", "日本水果"],
        "desc": "南部在地人最信賴的連鎖大型綜合商場，採購當季水果、茶飲、零食與日常物資。",
        "generation": GENERATION_DATA["san_a_shiozaki"]
    },
    {
        "id": "spot-ocean-tree",
        ...
        "defaultDurationMinutes": 60,
        "tags": ["核心基地", "面海別墅", "舒適客廳", "放鬆身心"],
        "desc": "旅程核心下榻別墅，位於沖繩南部臨海優美環境，早晚享受溫柔南國海風。",
        "generation": GENERATION_DATA["ocean_tree"]
    },
    {
        ...
        "tags": ["沖繩首店", "熟食熱濃湯", "石垣牛烤肉", "限定特產"],
        "desc": "2024全新開幕沖繩首間好市多！採買派對美食、沖繩黑糖點心、石垣牛肉排與限定商品。",
        "generation": GENERATION_DATA["costco"]
    },
    {
        ...
        "tags": ["週六限定煙火", "美式復古霓虹", "海濱棧道", "海景酒吧"],
        "desc": "每週六晚間施放璀璨海濱煙火！在復古美式街道與棕櫚海風中享受浪漫夜空。",
        "generation": GENERATION_DATA["american_village"]
    },
    {
        "id": "spot-itoman-fish",
        ...
        "defaultDurationMinutes": 90,
        "tags": ["現剖生魚片", "清甜魚湯", "焗烤海膽龍蝦", "大生蠔"],
        "desc": "南部海鮮朝聖地！厚切生魚片、特大生蠔、海膽杯與現烤龍蝦，產地直送銅板價！",
        "generation": GENERATION_DATA["itoman_fish"]
    },
    {
        ...
        "tags": ["Uniqlo大型店", "Loft生活雜貨", "2F海景露台", "免稅合併"],
        "desc": "沖繩南部最新型海濱購物中心！集結時尚服飾、Loft雜貨、KOJIMA電器與海景沙發露台。",
        "generation": GENERATION_DATA["iias"]
    },
    {
        ...
        "tags": ["數位光影", "透明玻璃步道", "近距離企鵝", "全室內平緩"],
        "desc": "尖端光影科技與海洋生態結合，腳踏懸空透明玻璃俯視熱帶魚群，水母圓柱光影療癒。",
        "generation": GENERATION_DATA["dmm_aquarium"]
    },
    {
        ...
        "defaultDurationMinutes": 45,
        "tags": ["700米白沙", "看客機起降", "遮陽涼亭", "防風慢步"],
        "desc": "南部最寬廣人工白沙灘，漫步平坦步道，近距離仰望那霸機場客機劃過海空的震撼景象。",
        "generation": GENERATION_DATA["chura_sun_beach"]
    },
    {
        ...
        "tags": ["奶油香煎魚", "琉球石灰岩建築", "鮮魚定食", "熱魚湯"],
        "desc": "在地老饕首選！招牌鐵板滋滋作響的時令奶油香煎鮮魚，外皮酥脆肉質細嫩，附鮮甜魚湯。",
        "generation": GENERATION_DATA["itoman_gyomin"]
    },
    {
        ...
        "tags": ["奧武山公園", "巨大紅色鳥居", "闔家平安御守", "平緩清幽"],
        "desc": "坐落奧武山綠意林蔭間，巨大朱紅鳥居與開闊平緩參道，晨間清幽祈福求平安。",
        "generation": GENERATION_DATA["gokoku_shrine"]
    },
    {
        ...
        "defaultDurationMinutes": 45,
        "tags": ["珊瑚礁斷崖", "海景神社", "小學生書包御守", "波之上海灘"],
        "desc": "琉球第一神社，朱紅本殿矗立在珊瑚斷崖之上！最具代表性的紅黑小學生書包交通御守。",
        "generation": GENERATION_DATA["naminoue_shrine"]
    },
    {
        "id": "spot-dfs-okinawa",
        ...
        "defaultDurationMinutes": 120,
        "tags": ["日本唯一路面免稅", "機場免提重物提貨", "一線名品專櫃", "頂級保養"],
        "desc": "全日本唯一境內路面免稅旗艦店！精品保養品一應俱全，回程直接於那霸機場管制區提貨。",
        "generation": GENERATION_DATA["dfs"]
    },
    {
        ...
        "defaultDurationMinutes": 90,
        "tags": ["奇蹟一英哩", "唐吉訶德", "紅芋塔現烤", "琉球工藝陶器"],
        "desc": "全長1.6公里的沖繩繁華主軸！伴手禮、風獅爺陶藝、現烤紅芋塔、唐吉訶德與巷弄小吃。",
        "generation": GENERATION_DATA["kokusai_dori"]
    },
    {
        "id": "spot-inaka-soba",
        ...
        "tags": ["400円銅板美食", "軟骨入口即化", "清醇柴魚湯", "市場巷弄"],
        "desc": "隱身市場巷弄的平價傳奇！400円軟骨肉燉至晶瑩透亮、布丁般入口即化，柴魚湯頭清甜甘醇。",
        "generation": GENERATION_DATA["inaka_soba"]
    },
    {
        "id": "spot-eibun-soba",
        ...
        "defaultDurationMinutes": 75,
        "tags": ["法式技藝跨界", "炙燒三層肉", "平板線上抽號", "柚子胡椒香檬"],
        "desc": "顛覆傳統的當代排隊第一名店！法餐背景主廚打造慢火甘醇高湯、厚切炙燒肉塊與特調佐料。",
        "generation": GENERATION_DATA["eibun_soba"]
    },
    {
        "id": "spot-sports-depo",
        ...
        "defaultDurationMinutes": 60,
        "tags": ["超大平面商場", "Coleman露營裝備", "減震健走鞋", "免稅退稅"],
        "desc": "那霸規模最大戶外運動量販！超值健走鞋、露營裝備與戶外機能服飾，平面寬敞好逛。",
        "generation": GENERATION_DATA["sports_depo"]
    },
    {
        "id": "spot-senagajima",
        ...
        "defaultDurationMinutes": 75,
        "tags": ["小希臘純白階梯", "幸福鬆餅", "飛機近距越頂", "金色落日海景"],
        "desc": "依傍海島階梯建起的純白地中海聚落！欣賞金色夕陽灑滿海面，客機低空掠過降落的夢幻奇景。",
        "generation": GENERATION_DATA["senagajima"]
    },
    {
        "id": "spot-maxvalu",
        ...
        "defaultDurationMinutes": 45,
        "tags": ["24小時營業", "產地草莓蘋果", "晚間特價熟食", "生活伴手禮"],
        "desc": "永旺AEON旗下24小時生活超市！採購新鮮日本水果、牛奶、宵夜熟食特價品與日用品。",
        "generation": GENERATION_DATA["maxvalu"]
    },
    {
        ...
        "defaultDurationMinutes": 45,
        "tags": ["達摩不倒翁牆", "安產長壽祈願", "親手點睛許願", "靜謐少階梯"],
        "desc": "首里古寺禪宗秘境！殿前排滿信徒許願的紅色達摩不倒翁，祈求健康長壽、安產開運。",
        "generation": GENERATION_DATA["daruma_temple"]
    },
    {
        ...
        "tags": ["首里城瓦頂外觀", "洗手間歇腿", "熱飲提神冰沙", "自駕中繼站"],
        "desc": "融入首里城下町古風的特色門市，中途如廁、熱飲提神小憩，活動筋骨好出發。",
        "generation": GENERATION_DATA["seven_eleven_shuri"]
    },
    {
        "id": "spot-jojoen",
        ...
        "defaultDurationMinutes": 75,
        "tags": ["西海岸無敵海景", "東京燒肉王者", "特選和牛午間套餐", "極品牛舌"],
        "desc": "落地窗面朝無邊蔚藍西海岸！品味油花勻稱的特選和牛五花、薄切牛舌與精緻甜點，尊榮舒適。",
        "generation": GENERATION_DATA["jojoen"]
    },
    {
        "id": "spot-parco-city",
        ...
        "tags": ["250間潮流店舖", "面海休憩沙發區", "無印良品旗艦", "外國人退稅中心"],
        "desc": "沖繩規模最大海濱商城！無印良品、Loft、3COINS、流行服飾，每層樓設有海景沙發歇腳。",
        "generation": GENERATION_DATA["parco_city"]
    },
    {
        ...
        "defaultDurationMinutes": 60,
        "tags": ["平價戶外機能", "防風防雨風衣", "輕量防滑健走鞋", "工裝穿搭"],
        "desc": "日本國民爆紅機能之王！防潑水風衣僅1900-2900円，長輩防風保暖、年輕人工裝露營首選。",
        "generation": GENERATION_DATA["workman_plus"]
    },
    {
        "id": "spot-katsunoya",
        ...
        "defaultDurationMinutes": 60,
        "tags": ["熟成厚切炸豬排", "現磨白芝麻香", "白飯高麗菜絲免費續", "室內停車場直通"],
        "desc": "新都心Main Place內人氣豬排！熟成厚切裹生麵包粉酥脆金黃，白飯味噌湯高麗菜免費續添。",
        "generation": GENERATION_DATA["katsunoya"]
    },
    {
        ...
        "defaultDurationMinutes": 75,
        "tags": ["純種阿古豬", "低溫慢炸熟成", "玫瑰粉嫩肉質", "老饕私藏居酒屋"],
        "desc": "老饕才懂的極品炸豬排！純種阿古豬低溫慢炸，肉質粉嫩多汁油脂甜美，蘸海鹽山葵回味無窮。",
        "generation": GENERATION_DATA["tonkatsu_koyaji"]
    },
    {
        ...
        "defaultDurationMinutes": 45,
        "tags": ["日本最西端道之驛", "農家清晨現摘", "柴燒古法黑糖", "芒果雪酪冰淇淋"],
        "desc": "日本最西端之道之驛！館內「菜菜色畑」販售清晨採摘生鮮、古法柴燒黑糖、海藻小農伴手禮。",
        "generation": GENERATION_DATA["toyosaki_roadside"]
    },
    {
        "id": "spot-aeon-naha",
        ...
        "defaultDurationMinutes": 120,
        "tags": ["單軌小祿站直通", "專屬免稅櫃台", "沖繩伴手禮總匯", "推車直通後車廂"],
        "desc": "離機場僅8分鐘車程！雪鹽餅乾、辣蝦餅、黑糖、泡盛一站免稅購足，室內大型停車場推車方便。",
        "generation": GENERATION_DATA["aeon_naha"]
    },
    {
        "id": "spot-tondo-ramen",
        ...
        "defaultDurationMinutes": 60,
        "tags": ["經典女人麵鹽味清湯", "男人麵黑蒜濃豚骨", "無限量免費辣豆芽", "食券投幣機"],
        "desc": "傳奇拉麵始祖名店！清雅甘甜的久米島天然鹽味「女人麵」與黑蒜濃郁「男人麵」雙重合奏。",
        "generation": GENERATION_DATA["tondo_ramen"]
    },
    {
        ...
        "defaultDurationMinutes": 120,
        "tags": ["國際線報到", "DFS免稅管制區提貨", "機場免稅店生巧", "滿載平安返台"],
        "desc": "辦理登機託運，管制區內提領DFS免稅品，採買Royce生巧克力與紅芋塔，攜帶南國蔚藍回憶登機。",
        "generation": GENERATION_DATA["naha_airport"]
    }
]

# Preset templates for Itinerary Planner
PRESET_ITINERARIES = {
    "official_5d": {
        "title": "官方推薦 5天4夜自駕經典",
        ...
        "days": [
            {
                ...
                "startTime": "11:00",
                "spotIds": ["spot-toyota", "spot-san-a-shiozaki", "spot-ocean-tree", "spot-costco-nanjo", "spot-ocean-tree", "spot-american-village", "spot-ocean-tree"]
            },
            {
                "day": 2,
                "title": "南部海味・光影水族館・漁民食堂煎魚",
                "startTime": "08:30",
                "spotIds": ["spot-ocean-tree", "spot-itoman-fish", "spot-ocean-tree", "spot-iias-toyosaki", "spot-dmm-aquarium", "spot-chura-sun-beach", "spot-itoman-gyomin"]
            },
            {
                "day": 3,
                "title": "雙神社參拜・DFS免稅・文青EIBUN・瀨長島落日",
                "startTime": "08:30",
                "spotIds": ["spot-ocean-tree", "spot-gokoku-shrine", "spot-naminoue-shrine", "spot-dfs-okinawa", "spot-kokusai-dori", "spot-inaka-soba", "spot-eibun-soba", "spot-sports-depo", "spot-senagajima", "spot-maxvalu", "spot-ocean-tree"]
            },
            {
                "day": 4,
                "title": "達磨寺祈福・敘敘苑海景燒肉・PARCO CITY 5小時・雙豬排巡禮",
                "startTime": "09:00",
                "spotIds": ["spot-ocean-tree", "spot-daruma-temple", "spot-seven-eleven-shuri", "spot-jojoen", "spot-parco-city", "spot-workman-plus", "spot-katsunoya", "spot-tonkatsu-koyaji", "spot-ocean-tree"]
            },
            {
                "day": 5,
                "title": "道之驛小農・永旺免稅衝刺・通堂拉麵經典・還車返台",
                "startTime": "09:00",
                "spotIds": ["spot-ocean-tree", "spot-toyosaki-roadside", "spot-aeon-naha", "spot-tondo-ramen", "spot-toyota", "spot-naha-airport"]
            }
        ]
    },
    "senior_3d": {
        ...
        "days": [
            {
                ...
                "startTime": "11:00",
                "spotIds": ["spot-toyota", "spot-san-a-shiozaki", "spot-ocean-tree", "spot-costco-nanjo", "spot-ocean-tree"]
            },
            {
                "day": 2,
                "title": "糸滿產地鮮甜海鮮・iias海景沙發放鬆・漁民食堂時令煎魚",
                "startTime": "09:00",
                "spotIds": ["spot-ocean-tree", "spot-itoman-fish", "spot-iias-toyosaki", "spot-dmm-aquarium", "spot-itoman-gyomin", "spot-ocean-tree"]
            },
            {
                "day": 3,
                "title": "護國神社晨行・波上宮求平安御守・敘敘苑軟嫩和牛・道之驛純黑糖・還車返台",
                "startTime": "09:00",
                "spotIds": ["spot-ocean-tree", "spot-gokoku-shrine", "spot-naminoue-shrine", "spot-jojoen", "spot-toyosaki-roadside", "spot-tondo-ramen", "spot-toyota", "spot-naha-airport"]
            }
        ]
    },
    "young_3d": {
        ...
        "days": [
            {
                ...
                "startTime": "11:30",
                "spotIds": ["spot-toyota", "spot-costco-nanjo", "spot-ocean-tree", "spot-american-village", "spot-ocean-tree"]
            },
            {
                "day": 2,
                "title": "糸滿海膽焗烤龍蝦痛風早餐・DMM懸空玻璃水族館・EIBUN炙燒麵・瀨長島飛機日落",
                "startTime": "09:00",
                "spotIds": ["spot-itoman-fish", "spot-dmm-aquarium", "spot-eibun-soba", "spot-dfs-okinawa", "spot-senagajima", "spot-maxvalu"]
            },
            {
                "day": 3,
                "title": "達磨寺紅不倒翁美拍・敘敘苑海景燒肉・PARCO CITY血拼・小やじ阿古豬・通堂男人麵返台",
                "startTime": "09:30",
                "spotIds": ["spot-daruma-temple", "spot-jojoen", "spot-parco-city", "spot-workman-plus", "spot-tonkatsu-koyaji", "spot-tondo-ramen", "spot-toyota", "spot-naha-airport"]
            }
        ]
    },
    "cross_2d": {
        ...
        "days": [
            {
                ...
                "startTime": "10:30",
                "spotIds": ["spot-toyota", "spot-itoman-fish", "spot-iias-toyosaki", "spot-dmm-aquarium", "spot-itoman-gyomin", "spot-ocean-tree"]
            },
            {
                "day": 2,
                "title": "波上宮海景祈願・敘敘苑落地海景頂級燒肉・瀨長島純白落日・通堂雙麵返台",
                "startTime": "09:00",
                "spotIds": ["spot-ocean-tree", "spot-naminoue-shrine", "spot-jojoen", "spot-parco-city", "spot-senagajima", "spot-tondo-ramen", "spot-toyota", "spot-naha-airport"]
            }
        ]
    }
}

CROSS_GEN_GUIDELINES = {
...
# Add SPOTS_CATALOG, PRESET_ITINERARIES, and CROSS_GEN_GUIDELINES to new_code
append_js = f"""

// ============================================================================
// SPOTS CATALOG (Comprehensive Unique Spots for Itinerary Planning)
// ============================================================================
const SPOTS_CATALOG = {json.dumps(SPOTS_CATALOG, ensure_ascii=False, indent=2)};

// ============================================================================
// PRESET ITINERARIES (Curated Cross-Generation Templates)
// ============================================================================
const PRESET_ITINERARIES = {json.dumps(PRESET_ITINERARIES, ensure_ascii=False, indent=2)};

// ============================================================================
// CROSS-GENERATION ROAD TRIP TRAVEL GUIDELINES
// ============================================================================
const CROSS_GEN_GUIDELINES = {json.dumps(CROSS_GEN_GUIDELINES, ensure_ascii=False, indent=2)};
"""

final_code = new_code + append_js

with open("data.js", "w", encoding="utf-8") as f:
    f.write(final_code)

print("data.js successfully enriched and written!")
